...
motif1=c(0.5,0,0,0.5,0,0) # G1 activates G2, G2 activates G3
motif2=c(0.5,0,0,0.5,-0.5,0) # G1 activates G2, G2 activates G3, G3 represses G1 (motif 1 + negative feedback)
motif3=c(0,0.5,0,0.5,0,0) # G1 and G2 independently activate G3
motif4=c(0,0.5,0,0.5,-0.5,-0.5) # G1 and G2 independently activate G3 and both are repressed by G3 (motif 3 + negative feedback)
motif5=c(-0.5,0.5,-0.5,0.5,0,0) # G1 and G2 independently activate G3 and repress each other (motif 3 + "competition")
motif6=c(0.5,0.5,0,0,0,0) # G1 activates G2 and G3
motif7=c(0.5,0.5,-0.5,0,-0.5,0) # G1 activates G2 and G3, and is repressed by G2 and G3 (motif 6 + negative feedback)
motif8=c(0.5,0.5,0,-0.5,0,-0.5) # G1 activates G2 and G3, G2 and G3 repress each other (motif 6 + "competition")

par.all=rbind(motif1,motif2,motif3,motif4,motif5,motif6,motif7,motif8)

width.all=rbind(c(0,0,1), # Width of fitness function for each ngene
	c(0,0,1),
	c(0,0,1),
	c(0,0,1),
	c(0,0,1),
...
T=1e5 # Duration of simulation for each lineage
T.rec=(1:(T/100))*100 # Time points at which results would be written into the output matrix
Nrep=500 # Number of lineages

out.all=matrix(0,nrow=nrow(par.all)*T/100,ncol=28)

row=1
for(npar in 1:nrow(par.all)){
	coeff=v2m(par.all[npar,])
	width=width.all[npar,]
	out.all[row:(row+T/100-1),1]=par.all[npar,1]
	out.all[row:(row+T/100-1),2]=par.all[npar,2]
	out.all[row:(row+T/100-1),3]=par.all[npar,3]
	out.all[row:(row+T/100-1),4]=par.all[npar,4]
	out.all[row:(row+T/100-1),5]=par.all[npar,5]
	out.all[row:(row+T/100-1),6]=par.all[npar,6]
	gt=list();pt=list()
	for(n in 1:Nrep){
