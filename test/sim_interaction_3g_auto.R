...
library(gtools) # The package is used to deal with permutations (combinations)
...
# Only combinations of -0.5,0,and 0.5 are considered because there would otherwise be too many combinations
permut.all=permutations(3,6,c(-0.5,0,0.5),repeats.allowed=TRUE)

# Convert a vector of interaction parameters to the corresponding 3x3 matrix
v2m <- function(v){
...
# Remove interaction parameter combinations that cannot produce a steady state phenotype
rm=rep(1,nrow(permut.all))
for(i in 1:nrow(permut.all)){
	pt=g2p(rep(1,6),v2m(permut.all[i,]))
	if(length(pt)==0){
		rm[i]=0
	}
}
